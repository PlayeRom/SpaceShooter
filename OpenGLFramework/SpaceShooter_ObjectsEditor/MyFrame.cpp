       ...
       : wxFrame( NULL, wxID_ANY, title, pos, size, style )
{
	...
	CreateStatusBar( 1 );
	SetStatusText( FILE_NAME_SHIPS_EDIT2 );

	m_pFileDialog = new wxFileDialog( NULL, wxFileSelectorPromptStr, wxEmptyString, _T("ships.dat"),
...
void CMyFrame::OpenFile( bool bIfCancelThenExit /*= false*/ )
{
	...
	m_pMyPanel->LoadUniqueShips( m_pFileDialog->GetPath() );
...
{
	wxString msgStr;
	msgStr.Printf( _T("OBJECTS EDITOR for Space Shooter project.\n")
		_T("Version: %s"), _T(VERSION ) );
